...
function getConfinementDay(birthDateStr, totalDays) {
  if (!birthDateStr) {
    return { day: 0, total: totalDays || 42, progress: 0, stage: STAGES[0], isEnded: false, rawDay: 0 };
  }
  ...
  return {
    ...
    total,
    progress,
    stage: { name: stage.name, maxDay: stage.maxDay === Infinity ? null : stage.maxDay },
    isEnded,
    rawDay: diff,
  };
}

function getTodayTips(birthDateStr, totalDays) {
  const info = getConfinementDay(birthDateStr, totalDays);
  const stage = STAGES.find((s) => info.day <= s.maxDay) || STAGES[STAGES.length - 1];
  return stage.tips;
}
